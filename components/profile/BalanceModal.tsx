...
function formatUZS(n: number) {
  return n.toLocaleString('ru-RU') + ' СЃСѓРј'
}
...
export function BalanceModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      addToast(`${dictionary.profile.topUpSuccess}: ${formatUZS(amount)}`, 'success')
      handleClose()
    } catch {
              ...
              <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-700">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    ...
                    <span className="font-semibold text-blue-600">{formatUZS(currentBalance)}</span>
                  </p>
                </div>
                <button
                  onClick={handleClose}
              ...
              <div className="px-6 py-5 flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {dictionary.profile.amount} (СЃСѓРј)
                  </label>
                  ...
                  {raw && !isValid && (
                    <p className="text-xs text-red-500">
                      {amount < MIN
                        ? `${dictionary.profile.minimum}: ${formatUZS(MIN)}`
                        : `${dictionary.profile.maximum}: ${formatUZS(MAX)}`}
                    </p>
                  )}
                  ...
                  {isValid && (
                    <p className="text-xs text-green-600 dark:text-green-400">
                      ...
                      <span className="font-semibold">{formatUZS(currentBalance + amount)}</span>
                    </p>
                  )}
                </div>
              </div>
