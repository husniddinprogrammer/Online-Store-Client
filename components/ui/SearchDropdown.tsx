...
import { products as productsApi } from '@/lib/api/endpoints'
...
export function SearchDropdown({ query, lang, onSelect, onClose }: SearchDropdownProps) {
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () =>
      productsApi.getProducts({ search: debouncedQuery, size: 8 }).then((r) => r.data.data),
      ...
      >
        {isFetching && results.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-2 text-slate-400">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-600">«{debouncedQuery}» bo&apos;yicha hech narsa yo&apos;q</p>
          </div>
        ) : (
          <div className="p-2">
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-shrink-0 text-right">
                      <p className={`text-sm font-bold ${hasDiscount ? 'text-blue-600 dark:text-blue-400' : 'text-slate-800 dark:text-slate-100'}`}>
...
function HighlightMatch({ text, query }: { text: string; query: string }) {
  ...
  return (
    <>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <mark key={i} className="bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded px-0.5 not-italic">
            {part}
          </mark>
        ) : (
