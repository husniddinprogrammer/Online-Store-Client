    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {notifications.map(
              (notification: NotificationResponse & { createdAt?: string }) => {
                const typeStyle = TYPE_STYLES[notification.type] ?? TYPE_STYLES.INFO
                return (
                  <div
                    key={notification.id}
                    onClick={() =>
                      !notification.isSeen && handleMarkSeen(notification.id)
                    }
                  ...
                  >
                    <span
                      className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${typeStyle.iconBg}`}
                    >
                      {typeStyle.icon}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm leading-snug ${notification.isSeen ? 'text-gray-600 dark:text-gray-400' : 'text-gray-900 dark:text-gray-100 font-medium'}`}
                      >
                        {notification.text}
                      </p>
