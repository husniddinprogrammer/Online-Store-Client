...
import type { AddressResponse } from '@/services/api/types'
...
const REGION_CITIES: Record<string, { value: string; label: string }[]> = {
  TASHKENT_CITY: [{ value: 'TASHKENT', label: 'Toshkent' }],
  TASHKENT_REGION: [
...
const selectCls =
  'w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm'
...
function AddressForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSave)} className="grid grid-cols-2 gap-3 mt-3">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-gray-600 dark:text-gray-400">
          {dict.profile.homeNumber}
        </label>
        <input {...register('homeNumber', { required: true })} className={selectCls} />
      </div>

      {/* Room number */}
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-gray-600 dark:text-gray-400">
          {dict.profile.roomNumber}
        </label>
        <input {...register('roomNumber', { required: true })} className={selectCls} />
      </div>
...
export default function PersonalDataPage({ params }: PageProps) {
  ...
  const initials = me ? `${me.name.charAt(0)}${me.surname.charAt(0)}`.toUpperCase() : '?'
          ...
          {meLoading ? (
            <div className="flex flex-col gap-3">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="h-10 animate-pulse bg-gray-200 dark:bg-gray-700 rounded-xl"
                />
              ))}
            </div>
          ) : (
          ...
          {addrLoading ? (
            <div className="flex flex-col gap-3">
              {[...Array(2)].map((_, i) => (
                <div
                  key={i}
                  className="h-16 animate-pulse bg-gray-200 dark:bg-gray-700 rounded-xl"
                />
              ))}
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/40">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-gray-800 dark:text-gray-200">
                          {addr.regionType}, {addr.cityType},{' '}
                          {dict.profile.homeNumber.toLowerCase()} {addr.homeNumber},{' '}
                          {dict.profile.roomNumber.toLowerCase()} {addr.roomNumber}
                        </span>
                      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 shadow-lg p-6 text-white">
          ...
          <p className="text-3xl font-bold mb-4">
            {meLoading ? '...' : formatCurrency(me?.balance ?? 0, lang)}
          </p>
          <button
          ...
          {meLoading ? (
            <div className="flex flex-col gap-3">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="h-6 animate-pulse bg-gray-200 dark:bg-gray-700 rounded-xl"
                />
              ))}
            </div>
          ) : (
