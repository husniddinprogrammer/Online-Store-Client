...
import { cart as cartApi } from '@/services/api/endpoints'
import type { AddToCartPayload, UpdateCartItemPayload } from '@/services/api/endpoints'
...
export function useRemoveCartItem() {
  ...
  return useMutation({
    mutationFn: (itemId: number) => cartApi.removeCartItem(itemId).then((r) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cart'] })
    },
  })
